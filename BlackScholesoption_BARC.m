...
datestr = BARC.textdata(2:end, 1);
dates = datenum(datestr, 'dd mmm yyyy');

figure(33)
hold on
plot(dates, BARC_ACP, 'g')
ylabel('Share Prices (GBP)')
xlabel('Time (Months)')
datetick('x', 'mmmyy', 'keepticks')
title("Barclays plc's Share Prices")


time=126;
time_tilde=1; 

LOGR=zeros(length(BARC_ACP)-1,1);
for i=1:length(BARC_ACP)-1
LOGR(i)=log(BARC_ACP(i+1))-log(BARC_ACP(i)); 
end
...
p=BARC_ACP(end,:); % P(T): Latest price of the historical data
...
p=BARC_ACP(end,:); % Current price
e=145; % Exercise price (lower)
