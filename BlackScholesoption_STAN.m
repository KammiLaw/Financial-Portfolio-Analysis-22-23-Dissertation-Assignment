...
figure (29)
hold on
plot(dates,STAN_ACP,'r')
ylabel('Share Prices (GBP)')
xlabel('Time (Months)')
datetick('x','mmmyy','keepticks')
title("Standard Chartered PLC's Share Prices")
...
LOGR=zeros(length(STAN_ACP)-1,1);
for i=1:length(STAN_ACP)-1
LOGR(i)=log(STAN_ACP(i+1))-log(STAN_ACP(i)); % log return for each day where time_tilde=1 day
end
...
% number of simulations random vectors
...
p=STAN_ACP(end,:); % P(T): Latest price of the historical data
...
p=STAN_ACP(end,:); % Current price
e=590; % Exercise price (lower)
...
profitcall=max(finalprice-e,0); 
profitput=max(e-finalprice,0);
profit1=profitcall-profitput-cost; % deducted from profits
