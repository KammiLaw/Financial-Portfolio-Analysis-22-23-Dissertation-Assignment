...
datestr=HSBA.textdata(2:end,1);
dates=datenum(datestr,'yyyy-mm-dd');


figure (32)
hold on
plot(dates,HSBA_ACP,'c')
ylabel('Share Prices (GBP)')
xlabel('Time (Months)')
datetick('x','mmmyy','keepticks')
title("HSBC Holding plc's Share Prices")


time=126;
time_tilde=1; 

LOGR=zeros(length(HSBA_ACP)-1,1);
for i=1:length(HSBA_ACP)-1
LOGR(i)=log(HSBA_ACP(i+1))-log(HSBA_ACP(i)); % log return for each day where tau_tilde=1 day
end
...
p=HSBA_ACP(end,:); % P(T): Latest price of the historical data
...
p=HSBA_ACP(end,:); % Current price
e=575; % Exercise price (lower)
