...
datestr=NWG.textdata(2:end,1);
% turn the dates into "double" rather than a "string". Note that if your
% dates are in a different format you need to adjust for this
dates=datenum(datestr,'yyyy-mm-dd');


figure (30)
hold on
plot(dates,NWG_ACP,'b')
ylabel('Share Prices (GBP)')
xlabel('Time (Months)')
datetick('x','mmmyy','keepticks')
title("Natwest Group plc's Share Prices")


time=126;
% assume that the option we hold lasts for 6 months (252/2)


time_tilde=1; %time_tilde is 1 trading day

LOGR=zeros(length(NWG_ACP)-1,1);
for i=1:length(NWG_ACP)-1
LOGR(i)=log(NWG_ACP(i+1))-log(NWG_ACP(i)); % log return for each day where time_tilde=1 day
end
...
p=NWG_ACP(end,:); % P(T): Latest price of the historical data
...
p=NWG_ACP(end,:); % Current price
e=245; % Exercise price (lower)
