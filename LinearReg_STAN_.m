% Linear regression plot for STAN vs the market FTSE 100
...
plot(Rm,Ri);% STAN return vs market return - plot per working day
hold on
plot(RET(:,1),RET(:,2),'ro'); 
xlabel('R_m(t)')
ylabel('R_{a}(t)')
legend('Optimal regression line','(FTSE,STAN) returns per working day')
title('Linear regression plot for STAN vs market')
%Noise coefficient-use these to determine how well the model performs
%versus the data
